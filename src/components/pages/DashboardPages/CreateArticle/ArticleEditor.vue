...
export default defineComponent({
  ...
  setup() {
    ...
    const categoryMapReverse = Object.fromEntries(
      Object.entries(categoryMap).map(([de, en]) => [en, de])
    );
    ...
    const createUpdateForm = (): FormData => {
      ...
      const postData = {
        ...
        quiz: articleQuiz.value.id || articleQuiz.value.questions?.length > 0 ? {
    ...
    const loadDraft = (draft: Draft) => {
      try {
        ...
        if (articleTitle.value.trim() !== "" && draft.id !== currentDraftId.value) {
          if (!confirm("Wenn sie einen neuen Post laden, gehen alle ungespeicherten Änderungen verloren. Sind sie sicher?")) {
            return;
          }
        }
