    ...
    const togglePostStatus = async (post: Post) => {
      try {
        ...
        if (err.response?.status === 403) {
          alert('Sie haben keine Berechtigung, diesen Artikel zu veröffentlichen. Nur der Autor kann seine eigenen Artikel veröffentlichen.');
        } else {
...
.loading-container, .no-results, .error-container {
  display: flex;
  flex-direction: column;
  align-items: center;
